  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (authError) {
        ...
        setError(authError.message || "Wrong email or password.");
        setBusy(false);
        return;
      }
      ...
      if (data?.session) {
        ...
        setError("Sign in succeeded but session cookies could not be set.");
        setBusy(false);
      }
    } catch (err: unknown) {
    ...
    <form onSubmit={onSubmit} className="mt-6 space-y-4">
      <div>
        ...
        <input
          ...
          placeholder="safatuzen24250@gmail.com"
      ...
      {error && (
        <p role="alert" className="rounded-xl bg-danger/10 p-3 text-sm text-danger font-medium">
          {error}
        </p>
      )}
    ...
    <div className="flex min-h-screen items-center justify-center bg-bg p-4">
      <div className="w-full max-w-sm rounded-3xl border border-hairline bg-surface p-8 shadow-lift">
        ...
        <p className="mt-4 text-center text-xs leading-relaxed text-ink-soft">
          ...
          Run <code className="rounded bg-blush px-1 py-0.5">node scripts/create-admin.mjs</code> to sync credentials.
        </p>
      </div>
    </div>
  );
